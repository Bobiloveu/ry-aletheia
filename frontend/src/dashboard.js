...
function dashboardMarkup() {
  ...
  documentNode
    .querySelectorAll('a[href="/case-library.html"]')
    .forEach((link) => {
  ...
  const nav = documentNode.querySelector("aside nav");
  if (nav && !nav.querySelector('a[href="/deployment.html"]')) {
    const deployment = documentNode.createElement("a");
    deployment.href = "/deployment.html";
    deployment.innerHTML = '<span aria-hidden="true">◇</span>部署建图';
    nav.insertBefore(
      deployment,
      nav.querySelector('a[href="/live-observation.html"]'),
    );
  }
  ...
  setup() {
    ...
    function routeLegacyPage(event) {
      ...
      if (window.location.port === "5173") {
        ...
        if (target === "/runtime-settings.html")
          window.location.assign("/runtime-settings.html");
