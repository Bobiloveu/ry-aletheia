...
- 电梯锚点必须引用 `elevator` 组件；导出坐标由该组件中心派生，机器人朝向取电梯门外法线的反方向（组件 `yaw` 为局部 X 轴，不能直接作为车头朝向），页面不允许手填覆盖。门禁、闸机等组件也可作为组件中心锚点；不具备明确中心语义时使用人工 `waypoint`。
...
- 当前地图画布始终绘制一个低干扰的原点十字坐标系：`X`、`Y` 轴及朝向标记随地图坐标变换，不随屏幕旋转；其只读标签显示精确坐标与朝向。
- 原点是只读地图事实，不能拖动或修改。
...
- 仅导出项目快照中的地图、PGM、批准定位 YAML 模板，以及定位加载器实际索引的 `index.txt`、静态 PCD 和可选动态 PCD；索引缺失或任一索引块缺失均阻断导出。产物继续位于项目 `exports/` 的实验包中，`robot_runtime_changed` 保持 `false`。
