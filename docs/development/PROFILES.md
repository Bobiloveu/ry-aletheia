...
部署页面的 `indoor_elevator_v1` 任务编译器目前只支持**室内两图、单电梯、往返**的实验配置：大厅图与目标层图各一张、同一物理电梯、一个起点和一个带门牌号的目标点。实施人员只标记实际组件及其属性（地图角色、楼栋/单元/楼层、电梯 ID/门向/候梯距离、起点、目标门牌）；路线、交接点和地图转场由编译器推导，不应人为编排。
