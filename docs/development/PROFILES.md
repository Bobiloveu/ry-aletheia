...
部署页面的 `indoor_elevator_v1` 任务编译器目前只支持**室内两图、单电梯、往返**的实验配置：大厅图与目标层图各一张、同一物理电梯、一个起点和一个带门牌号的目标点。实施人员在第一张图新建物理电梯并填写编号、协议与服务楼层；后续楼层只关联同一编号，再分别确认本层的门向、尺寸和候梯距离。地图角色、楼栋/单元/楼层、起点和目标门牌仍在各自地图上维护；路线、交接点和地图转场由编译器推导，不应人为编排。

“预览”与“下载”只在项目自己的实验导出目录生成任务 JSON、行为树 XML、定位 YAML 与 ZIP；它们不写入机器人任务目录、行为树运行目录或定位运行配置，也不会启动 ROS、Supervisor 或下发任务。任何产物都必须逐项对照已批准样例，并在本地任务执行器中验证后，才能由现场维护流程另行决定是否安装。

PC 部署工作台已经可以把户外图、摆渡层、电梯大厅和用户楼层配置成可拖拽的阶段流程；但实验任务编译器目前仍只批准室内两图、单电梯、往返模板。三图室外/大厅转场、摆渡层实际动作，以及闸机、自动门、窄通道等其他设备组件尚未有已批准的行为树模板，编译器必须拒绝这些场景，不能猜测生成机器人动作。
