...
- 多地图轨迹、理想路径与虚拟墙证据，以及 HTML/CSV 离线报告。报告内嵌离线轨迹交互，鼠标沿折线移动会磁吸最近采样点，并在点上方显示北京时间、路线、坐标和采样序号。
- PC 部署工作台用可视化阶段流程配置户外图、摆渡层、电梯大厅和用户楼层，支持拖拽排序与自动吸附；项目快照由 Backend 校验并可安全删除，Mobile 不调用该编辑能力。
