...
已由维护人员启用工业相机配置时，页面下方会自动出现 0–6 张“低延迟相机流”卡片。它们通过 WebRTC 直接从小车的私有 MediaMTX 获取 H.264 视频，不占用点云或位姿遥测链路，也不会向小车发送控制指令。卡片显示“等待相机”表示网关已就绪、但 ROS 图像尚未进入编码器；不要因此重启相机节点。
...
执行该用例前，工具会自动应用方案；测试完成、终止或人工恢复结束后，会先恢复常规启动脚本，再重启并确认相关依赖稳定运行。只有这两步都成功，页面才显示已恢复；若恢复失败会保留待恢复提示，先按提示排除 Supervisor/权限问题后重试。不要在测试过程中手动修改对应启动脚本，也不要在页面显示待恢复时退出或升级控制台。
