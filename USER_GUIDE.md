...
已由维护人员启用工业相机配置时，页面下方会自动出现 0–6 张“低延迟相机流”卡片。它们通过 WebRTC 直接从小车的私有 MediaMTX 获取 H.264 视频，不占用点云或位姿遥测链路，也不会向小车发送控制指令。卡片显示“等待相机”表示编码器尚未收到输入。目标检测和分割卡片表示 ROS 图像尚未进入编码器；前、后、左、右物理相机的 ShmSDK 试运行则表示尚未从对应 `Cam*` 通道取得新图像。两种情况都不要因此重启原相机节点或修改 `mempool`；先进入“工具日志”导出诊断信息给维护人员。
...
当前 2.3.8 仅对前、后、左、右四路物理相机进行 ShmSDK 接入试运行；目标检测和可通行区域分割没有改动。若维护人员要求结束试运行或出现持续黑屏、明显花屏、视频进程异常退出等情况，使用维护人员提供的已签名 `ry-aletheia_2.3.7.zip` 回退包，操作与普通升级完全相同：确认无执行中或待恢复测试 → 在“运行配置”上传 ZIP → 点击“校验并应用升级” → 刷新页面确认版本回到 2.3.7。

回退过程会先把当前 2.3.8 程序保存为唯一备份，因此必要时仍可恢复到试运行版本。不要手工复制程序文件、不要解压或自行重打包 ZIP、不要为了回退去停止 `mempool`、相机驱动、导航或 Supervisor。回退后由维护人员核对四路物理相机是否重新显示为 ROS 输入；试运行记录和停止条件见 [ShmSDK 视频接入试运行记录](docs/SHMSDK_VIDEO_TRIAL_2.3.8.md)。
