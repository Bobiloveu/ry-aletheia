...
完整离线 DEB 已内置本工具专用的 Foxglove Bridge、MediaMTX、GStreamer 和 Intel VAAPI 视频运行时，推荐始终使用 `dpkg -i` 安装。安装过程不会在小车上执行 `apt install`。请勿对完整包执行 `apt autoremove`；如 `dpkg` 提示该 DEB 自身缺少依赖，再将完整错误信息提供给维护人员。
...
实时运行观测是只读页面，不会发送驾驶、导航或任务控制指令。PC 端地图独占实时工作区，从低到高依次叠加静态地图、红色虚拟墙、青色车体和紫色激光点云；不再保留与 WebRTC 重复的 Foxglove 图像订阅窗。顶部绿色“已连接”表示已连接工具专用 Bridge，状态栏会显示地图、定位和点云的等待或接收情况。

已由维护人员启用工业相机配置时，页面下方会自动出现 0–5 张“低延迟相机流”卡片。它们通过 WebRTC 直接从小车的私有 MediaMTX 获取 H.264 视频，不占用 Foxglove 图像链路，也不会向小车发送控制指令。卡片显示“等待相机”表示网关已就绪、但 ROS 图像尚未进入编码器；不要因此重启相机节点。
...
4. WebRTC 相机默认关闭。首次开通由维护人员核对相机话题、`/dev/dri/renderD128` 权限、`ros_domain_id` 和现场网络后，日常只需在本页“低延迟相机流”右上角点击“启用视频/关闭视频”。首次启用会自动选中这套已验证的五路相机；之后若维护人员只保留部分相机，页面会保留该选择。关闭会立即回收本工具的 MediaMTX 与视频旁路进程，避免占用编码与网络资源；不会操作车辆节点。`ry-aletheia-video` 仅用于维护排障。
