...
部署项目中的地图阶段也在 PC 部署工作台内配置：先添加需要的“户外图”“摆渡层”“电梯大厅”“用户楼层”卡片，再拖动卡片调整顺序。拖动到卡片前方、后方或两个卡片之间时，蓝色插入指示会自动吸附到可放置位置；松开后保存流程。流程必须包含一个电梯大厅和一个用户楼层，用户楼层必须最后，摆渡层可以按现场实际需要重复添加。项目列表中的删除按钮会要求确认，删除项目快照及其地图/导出文件，不能恢复。
