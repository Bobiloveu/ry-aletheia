...
默认视觉语言为克制的石墨色信息界面：紧凑、清晰，并在操作压力下保持冷静。日光配色使用白蓝工作表面，在明亮工作场景中保持原生、冷静和清晰的层级。它不是营销界面，不使用装饰性渐变、玻璃质感层或自主循环动效。观测能力与受控的车辆操作能力保持架构边界。
...
| 角色 | Token | 值 |
| --- | --- | --- |
| App 背景 | `canvas` | `#F5F8FC` |
| 标准表面 | `surface` | `#FFFFFF` |
| 抬升控制表面 | `surfaceRaised` | `#EDF3FA` |
| 下沉导航与安静区块 | `surfaceSunken` / `surfaceMuted` | `#E7EEF8` / `#F0F5FB` |
| 主文本 | `textPrimary` | `#172033` |
| 次文本 | `textSecondary` | `#4E5C70` |
| 辅助文本 | `textTertiary` | `#748298` |
| 标准边框 | `border` | `#CAD6E4` |
| 主操作与中性信号 | `cyan` | `#0A63C4` |
| 成功 / 警告 / 错误 | `mint` / `warning` / `danger` | `#137B68` / `#8C5B08` / `#B5423B` |

日光模式把纯白限制在可交互或阅读表面，不把它用作整屏背景；主操作使用有足够对比度的白色前景，状态色仅表达真实状态，绝不作装饰。地图、PointCloud、机器人和虚拟墙图层在两套配色中保持可区分的语义色。
...
- 手动控制是独立的受控操作面：进入前必须二次确认，未获得车端会话、安全控制源和正常急停状态前，摇杆始终锁定。摇杆属于直接操控，拖动时 1:1 跟随手指、可随时打断：上/下为前后，左/右为转向，斜向为两轴叠加的转弯弧线；回到中心或松手均立即请求 STOP。控制盘不使用教学性说明、罗盘箭头、渐变或发光；仅以可跟随的拇指和短促回中弹簧表达状态。常用速度直接展示，压力/加速度等低频高级参数放入下一层面板。不得在其附近放置会改变导航、ROS、地点或任务状态的操作。
