...
- 基础地图、米制 grid、局部代价地图、虚拟墙、点云、Pose/车体在同一坐标变换下对齐；检查 costmap 原点/朝向、色标及 `0`/`255` 的透明语义。
- 保持地图页面打开超过 5 秒后停止 costmap 输入，确认该层安全隐藏；恢复有效新帧后确认只显示最新一帧，且地图工具栏的显示/隐藏动作不影响其它遥测层。
