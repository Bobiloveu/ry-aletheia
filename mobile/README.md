...
- 连续摇杆提交受控的 `linear_ratio` / `angular_ratio`；上/下是前进/后退，左/右是左/右转，右上为前进并右转的弧线而非横向平移。保持输入按现有看门狗要求刷新，中心、松手、状态未知和急停均请求 STOP；App 退后台、离页或断开连接只 RELEASE 本端会话，不改变其他 Web/App 的控制源。操作者明确选择退出手动控制时才执行全局 EXIT → navigation。
