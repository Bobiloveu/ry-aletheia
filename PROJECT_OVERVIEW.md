...
接收方仍必须自行安装匹配的 Ubuntu 22.04 `amd64` + ROS 2 Humble；最小包不是 ROS 发行版替代品。`cpp_sdk/` 不是仓库目录，也不被 `build_binary.sh` 读取，不能作为标准构建前置条件。`export_robot_cpp_sdk.sh` 仅保留给需要在无 ROS 开发包的异机构建机上进行原生 C++ 兼容性验证时使用；如确有该需求，应将其归档解压到独立临时目录并先验证 `cmake -S live_preprocessor -B build/live_preprocessor`。不要把 SDK 解压到工程根目录、`/opt/ros`，也不要覆盖系统 ROS。
