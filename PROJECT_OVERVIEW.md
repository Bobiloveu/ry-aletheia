...
> 面向维护、开发、构建与现场排障人员。项目入口见 [README.md](README.md)；日常安装、使用和升级请优先阅读 [USER_GUIDE.md](USER_GUIDE.md) 与 [GitHub Releases](https://github.com/Bobiloveu/ry-aletheia/releases)。模块边界与迁移状态见 [docs/architecture](docs/architecture/README.md)，跨端接口事实来源见 [shared/contracts](shared/contracts/README.md)，按角色准备工具链与开发命令见 [docs/development/PROFILES.md](docs/development/PROFILES.md)。
...
它负责测试编排、证据记录和只读观测；已获批准的 miniapp 手动控制是一个受控例外，但不替代小车原有的导航、定位、地图服务、物理急停或底盘安全控制。
...
- `config/console.json` 的 `vehicle_control` 仅保存手动驾驶 `press`、`movement_acc` 与 `stop_acc`。零 Twist 统一使用 `stop_acc`，非零 Twist 使用 `movement_acc`；这些参数绝不改变固定解除急停报文。`autostart_enabled` 仅控制当前普通用户的 `systemd --user`/XDG 启动入口，默认关闭；自启显式使用 `config/video.json` 的 ROS domain（无效时为目标车默认 `66`），不改变机器人节点生命周期。
...
当前移动端以 Observation / Monitoring、Test 和 Diagnostic 为主要能力，并增加一项受控例外：工具页“手动控制”只消费既有 `/api/vehicle-control/*` HTTP 契约，操作员二次确认后才可进入会话，且仅在车端确认控制源、会话和急停状态后解锁连续的“前后 + 转向”摇杆。Mobile 发送归一化比例，Backend 才负责换算并发布 Twist；右上表示前进并右转的弧线，不表示横向平移。PC Web 继续保持既有四方向 `/command` 控制。Mobile 不复用观测链路、不直接使用 ROS2 或 Twist，也不扩大底盘、导航或安全控制边界。未来若增加导航、地点、任务执行或其他 Robot Operation / Command，仍必须先形成独立、明确确认的后端命令契约，并具备权限、确认和审计模型。
...
| 部署验收 | 从正式任务目录只读生成冻结计划；`栋号 + 单元` 是物理楼宇单元，抽样显示实际覆盖，不以未覆盖每层/每户作为异常。场景方案与 Supervisor 编排均为可选的计划级运行准备，页面可见其冻结与执行阶段；未选择时严格沿用常规验收流程。 |
| 场景前置配置 | 选择 FCRP `.launch.py` 与 lightning YAML，受控替换 `handle_modules.sh`；应用/恢复均让受控依赖重读脚本并确认稳定运行。 |
| 多地图轨迹 | 缓存实际地图，叠加实际轨迹、理想路线、虚拟墙并归档；报告图包含离线磁吸游标、北京时区时间、路线、坐标和采样序号。 |
| 报告中心 | 统一生成、预览、下载、删除自动测试报告与部署验收报告。两类报告均为可离线查看的单文件 HTML；验收报告额外归档范围、覆盖、每项时间/结果和已验证的轨迹证据，轨迹交互磁吸采样点并在点上方显示北京时间、路线、坐标和采样序号。 |
| 实时运行观测 | 地图、虚拟墙、车体轮廓、定位、点云、PC 与 Flutter Mobile 的局部代价地图，以及网页按需启停的低延迟相机视频。 |
| 运行配置 | 依赖编排、车型、实时观测、升级、持久化设置和可选的用户级控制台自启。 |
...
专项测试不另建一套导航或任务协议，而是以既有任务 JSON 为场景资产、以既有 `/start_execute_tasks` 为唯一任务下发入口。电梯连续进出、门禁多次通行、多任务连续配送、往返运行和窄通道等场景均可绑定为用例并设置轮次、轮间隔与受控场景参数。正常轮次自动串行执行；一旦任务失败、预检失败或发生需人工处置的停滞，工具记录当前轮次和原因并进入 `awaiting_recovery`，由操作员将车辆恢复至安全起点后再继续剩余轮次，禁止在异常位置盲目自动重发。

每轮产生可追溯结果：开始时间、耗时、任务服务反馈、通过/失败、人工干预、地图/轨迹证据及电梯等待或停滞状态。计划摘要自动计算请求轮次、已完成、成功、失败、取消和通过率；HTML/CSV 报告汇总这些信息。失败定位以真实上游信息为准：工具可关联任务返回消息、当前路线/轨迹位置和运行状态，但不会在没有上游节点标识的情况下臆测“某个门禁”或“某一层”失败。门禁设备状态、通道物理宽度等独立判定源尚未接入。

### 2.2 任务编译（实验）

部署建图页提供 `indoor_elevator_v1` 实验编译器，但它不是任务安装或执行功能。当前仅支持一个大厅图、一个目标层图、同一物理电梯以及往返的室内两图单电梯场景。实施人员在首张图建立一次物理电梯的编号、协议与服务楼层，后续地图只关联同一电梯，并在各自地图标记门向和候梯距离；地图的楼栋/单元/楼层角色、起点和目标门牌也由实施人员维护。路线、衔接点、子任务顺序、速度模式和已批准行为树绑定由工具推导，不能要求现场人员手动绘制路线或转场。

预览/下载会将 JSON、行为树 XML、定位 YAML、清单和 ZIP 保留在部署项目自己的实验导出目录。它们绝不写入 `/opt/ry/data/tasks/origin_tasks`、行为树运行目录或定位运行配置，绝不启动 ROS/Supervisor，也绝不下发任务。现场使用前必须逐文件比对已批准样例，并通过本地任务执行器验证；是否安装到机器人必须走独立、人工确认的现场流程。

部署项目的 PC 工作台可以可视化配置户外图、重复摆渡层、电梯大厅和用户楼层的阶段顺序，并通过拖拽吸附调整流程；但 `indoor_elevator_v1` 实验编译器目前仍只批准室内两图、单电梯、往返模板。室外—大厅—楼层转场、摆渡层实际动作，以及闸机、自动门、窄通道等其他设备组件仍未提供已批准行为树模板，因此编译器必须拒绝这些场景，不能用推测行为替代真实模板。
...
RY Aletheia（普通账户）
  ...
  ├─ C++ 轻量局部代价地图进程：/local_costmap/costmap → 按 stamp 的 map←source 投影 → 回环 UDP 栅格帧（≤65535 cells）
  ├─ Aletheia 专用遥测网关：UDP 最新帧组装 → 三条 Binary WebSocket
  ├─ 可选视频运行时（由控制台拥有）
...
| `robot_logs.py` | 当前小车业务日志源的受控配置、非递归清单与原始文件下载校验。 |
...
节点名称、预检集、阶段顺序与等待时间必须由配置驱动，不能在业务代码中写死某一台车的节点名。新车的 Supervisor 监控集默认为空；未启用依赖编排时，空监控集不访问 Supervisor，也不得阻断常规测试或部署验收。
...
- 报告优先显示用例别名；轨迹图片和采样点数据内联，下载后仍可离线查看。鼠标靠近实际轨迹时磁吸最近点，卡片位于点上方，不计算或显示未经定义的“偏差”。
...
- 点云、位姿与局部代价地图由三个独立进程运行；costmap callback 只覆盖一条最新 `OccupancyGrid` 槽，TF/编码/网络发送由其 worker 完成，避免坐标转换或大栅格阻塞高频位姿。
...
这是私有、固定格式的数据通道，不是通用 ROS-Web Bridge。C++ 到网关的 UDP 分片以 `RALT` 魔数和版本号开头，携带流种类、流 ID、`frame_seq`、时间戳、`chunk_index`、`chunk_count`、记录数和 payload 长度；头部为 30 Byte，payload 上限为 1152 Byte。点云固定使用 `127.0.0.1:8769`，位姿固定使用 `127.0.0.1:8770`，局部代价地图固定使用 `127.0.0.1:8771`，各自有 socket、接收线程和 assembler。costmap `kind=3` 载荷为 map 原点 x/y/yaw、resolution、width/height 与最多 65,535 个原始 cell；`-1` 按原始 unsigned 值 `255` 传输。网关拒绝错误魔数/版本、截断包、非法分片号、超出分片或记录上限的包。分片可以乱序到达；重复分片不重复计数；新序号会立即淘汰未完成旧帧，残帧在短超时后清理。协议没有 ACK、重传、历史缓存或“补全旧帧”的等待逻辑。
...
现场排障优先看 `logs/live_preprocessor_cloud.log`、`logs/live_preprocessor_pose.log`、`logs/live_preprocessor_costmap.log` 和工具日志；再检查 `/collision_voxel_layer/points`（主流）、`/livox/lidar`（回退）、`/local_costmap/costmap` 与 `map → base_*`、`map → costmap.header.frame_id` TF 是否存在且持续更新。页面保持打开约 10 秒后，可从 `GET /api/observation` 的 `telemetry` 和 `client_metrics` 读取网关状态、点云/位姿/costmap 源年龄、接收频率、渲染帧率和长帧计数，以区分上游、车端预处理、网络和 PixiJS 图层更新问题。实车参考：预处理点云约 10Hz，前端为降低点云几何更新竞争主动消费约 8Hz；costmap 按上游发布节奏显示，不主动补发。
...
| 自启后控制源状态一直“正在读取” | 先比较控制台进程与车端节点的 `ROS_DOMAIN_ID`（目标车默认 `66`），再运行 `ROS_DOMAIN_ID=66 ros2 topic info -v /control_source_state`；点云/视频正常不代表控制源 Topic 同域。车端节点尚未发布时允许短暂 `pending/unknown`，但不得把空读数当作已确认状态。 |
