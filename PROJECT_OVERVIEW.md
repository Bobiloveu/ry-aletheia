...
部署建图页提供 `indoor_elevator_v1` 实验编译器，但它不是任务安装或执行功能。当前仅支持一个大厅图、一个目标层图、同一物理电梯以及往返的室内两图单电梯场景。实施人员只需标记真实组件和属性：地图的楼栋/单元/楼层角色、起点、目标门牌，以及两图电梯的 ID、门向和候梯距离；路线、衔接点、子任务顺序、速度模式和已批准行为树绑定由工具推导，不能要求现场人员手动绘制路线或转场。
