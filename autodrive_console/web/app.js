...
function renderDependencyEditor() {
  ...
  $('discoveredProcesses').innerHTML = supervisorProcesses.length ? supervisorProcesses.map(process => `<label class="process-choice ${selected.has(process.name) ? 'assigned' : ''}"><input class="monitor-node" type="checkbox" value="${escapeHtml(process.name)}" ${monitorNodes.includes(process.name) ? 'checked' : ''} title="运行依赖状态监控"><span class="choice-label">监控</span><input class="orchestration-node" type="checkbox" value="${escapeHtml(process.name)}" ${selected.has(process.name) ? 'disabled' : ''} title="加入启动编排"><span class="choice-label">编排</span><span class="node-state ${process.status === 'RUNNING' ? 'running' : 'bad'}">${escapeHtml(process.status)}</span><b>${escapeHtml(process.name)}</b><small>${escapeHtml(process.detail)}</small></label>`).join('') : '<p>点击“识别当前 Supervisor 节点”读取本机状态。</p>';
  $('dependencyFlow').innerHTML = dependencyPlan.steps.length ? dependencyPlan.steps.map((step, index) => `<div class="flow-step" data-step="${index}"><div class="flow-index">阶段 ${index + 1}<small>并行重启 · 等待全部 RUNNING</small></div><div class="flow-nodes">${step.nodes.map(name => `<span>${escapeHtml(name)}</span>`).join('')}</div><label>额外稳定等待 <input class="step-wait" type="number" min="0" max="300" value="${Number(step.wait_seconds || 0)}"> 秒</label><div class="flow-actions"><button class="move-step" data-direction="up" type="button" ${index ? '' : 'disabled'}>↑</button><button class="move-step" data-direction="down" type="button" ${index < dependencyPlan.steps.length - 1 ? '' : 'disabled'}>↓</button><button class="remove-step" type="button">×</button></div></div>${index < dependencyPlan.steps.length - 1 ? '<div class="flow-arrow">↓ 前置阶段就绪后继续</div>' : ''}`).join('') : '<p>尚未配置依赖阶段。</p>';
}
