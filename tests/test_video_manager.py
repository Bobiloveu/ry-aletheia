...
class VideoManagerTests(unittest.TestCase):
    ...
    def test_full_deb_reuses_embedded_runtime_without_copying_dependency_archives(self):
        deb_script = (ROOT / "build_deb_package.sh").read_text(encoding="utf-8")
        postinst = (ROOT / "packaging" / "debian" / "postinst").read_text(encoding="utf-8")
        binary_script = (ROOT / "build_binary.sh").read_text(encoding="utf-8")
        self.assertIn('--add-data "$VIDEO_RUNTIME:runtime/video"', binary_script)
        self.assertNotIn('build_video_runtime.sh" --output-dir "$VIDEO_RUNTIME"', deb_script)
        self.assertNotIn('cp -a "$VIDEO_RUNTIME"', deb_script)
        self.assertNotIn('PRIVATE_VIDEO=', postinst)
        self.assertNotIn('cp -a "$PRIVATE_VIDEO/." "$ROOT/runtime/video/"', postinst)
        self.assertIn('"$ROOT/runtime/video"', postinst)
        self.assertIn("-name '*.a'", deb_script)
        self.assertNotIn("'Depends:", deb_script)
        self.assertNotIn('apt-get', deb_script)
    ...
    def test_video_runtime_is_console_owned_and_has_a_manual_maintenance_launcher(self):
        ...
        console_launcher = (ROOT / "packaging" / "debian" / "ry-aletheia-launcher").read_text(encoding="utf-8")
        deb_script = (ROOT / "build_deb_package.sh").read_text(encoding="utf-8")
        postinst = (ROOT / "packaging" / "debian" / "postinst").read_text(encoding="utf-8")
        self.assertIn("start|stop|status|restart", launcher)
        self.assertIn("--video-runner", launcher)
        self.assertIn("$HOME/.bashrc", launcher)
        self.assertIn('config/video.json', console_launcher)
        self.assertIn('export ROS_DOMAIN_ID', console_launcher)
