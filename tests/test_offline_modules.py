...
class OfflineModuleTests(unittest.TestCase):
    ...
    def test_live_observation_frontend_subscribes_to_transient_map(self):
        ...
        self.assertIn("import { Application, Container, Graphics, Sprite, Texture } from 'pixi.js';", source)
        self.assertIn("async function initializePixiRenderer()", source)
        self.assertIn("new ResizeObserver(resizeCanvas).observe(interaction.parentElement);", source)
        ...
        self.assertIn("pixiMapTexture = Texture.from(mapRaster);", source)
